...
export function validateHarmonyTask(task: unknown): asserts task is HarmonyTask {
  ...
  if (typeof t.description !== 'string' || !t.description) throw new Error('description is required');
  ...
  if (!Array.isArray(t.answer) || t.answer.length === 0) throw new Error('answer must be a non-empty array');
  for (const a of t.answer) {
    const ans = a as Answer;
    if (!['musicxml', 'image', 'json'].includes(ans.type)) throw new Error('answer.type is invalid');
    if (typeof ans.data !== 'string' || !ans.data) throw new Error('answer.data is required');
  }
  if (t.difficulty && !['easy', 'normal', 'hard'].includes(t.difficulty)) throw new Error('difficulty is invalid');
